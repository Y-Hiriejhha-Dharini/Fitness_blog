    ...
    <section class="px-8 py-7">
        <main class="max-w-lg mx-auto bg-gray-100 border border-gray-200 rounded-xl">
            <form action="/login" method="POST" enctype="multipart/form-data">
                ...
                <div class="px-4 py-2">
                    <label for="images" class="block uppercase text-gray-700 font-bold text-sm mb-2">Image</label>
                    <input type="file" name="images" class="w-full border border-gray-100 p-2" required>
                    @error('images')
                        <p class="text-red-600 text-sm mt-2">{{$message}}</p>
                    @enderror
                </div>
